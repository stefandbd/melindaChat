...
const IMAVCallContainerView = props => {
  ...
  const requestAudioPermission = () => {
    return PermissionsAndroid.request(
      ...
      {
        title: IMLocalized('Need permission to access microphone'),
        message: IMLocalized(
          'To run this demo we need permission to access your microphone',
        ),
        buttonNegative: IMLocalized('Cancel'),
        buttonPositive: IMLocalized('OK'),
      },
    )
  }
  ...
  const requestCameraPermission = () => {
    return PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.CAMERA, {
      title: IMLocalized('Need permission to access camera'),
      message: IMLocalized(
  ...
  if (
    ...
    const callStatusLabelTitle =
      activeCallData.callType == 'video'
        ? activeCallData?.status == 'incoming'
          ? IMLocalized('Incoming Video Call...')
          : IMLocalized('Video Calling...')
        : activeCallData?.status == 'incoming'
        ? IMLocalized('Incoming Audio Call...')
        : IMLocalized('Audio Calling...')
